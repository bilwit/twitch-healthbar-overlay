...
export default function parseMessage(message: any): ParsedMessage | null {
  ...
  let parsedMessage: ParsedMessage = {  // Contains the component parts.
    tags: null,
    source: null,
    command: null,
    parameters: null
  ...
  if (msg[idx] === '@') {  // The message includes tags.
    let endIdx = msg.indexOf(' ');
    rawTagsComponent = msg.slice(1, endIdx);
  ...
  if (msg[idx] === ':') {
    idx += 1;
    let endIdx = msg.indexOf(' ', idx);
    rawSourceComponent = msg.slice(idx, endIdx);
    idx = endIdx + 1;  // Should point to the command part of the message.
  }

  // Get the command component of the IRC message.
  let endIdx = msg.indexOf(':', idx);  // Looking for the parameters part of the message.
  if (-1 == endIdx) {                      // But not all messages include the parameters part.
    endIdx = msg.length;                 
  }

  rawCommandComponent = msg.slice(idx, endIdx).trim();

  // Get the parameters component of the IRC message.
  if (endIdx != msg.length) {  // Check if the IRC message contains a parameters component.
    idx = endIdx + 1;            // Should point to the parameters part of the message.
    rawParametersComponent = msg.slice(idx);
  }
